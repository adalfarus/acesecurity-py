...
if _ty.TYPE_CHECKING:
    ...
    from .. import GenericLabeledEnum
...
__all__ = ["Backend", "Cipher", "SymCipher", "AsymCipher", "SingleSerializable", ""]


class Backend(enum.Enum):
    """One of the cryptography backends"""
    cryptography = ("acecurity.crypto._crypto", "Cryptography backend")
    pycryptodomex = ("acecurity.crypto._pycryptox", "PyCryptodomeX backend")
    quantcrypt = ("acecurity.crypto._quantcrypt", "Post-quantum via QuantCrypt")
    argon2_cffi = ("acecurity.crypto._argon2_cffi", "Argon2 via argon2_cffi")
    bcrypt = ("acecurity.crypto._bcrypt", "Bcrypt backend")
    # pynacl = ("acecurity.crypto._pynacl", "NaCl/libsodium backend")  # Unsupported
    # tink = ("acecurity.crypto._tink", "Google Tink backend")  # Unsupported
    # rust_crypto = ("acecurity.crypto._rust", "Rust-backed cryptographic backend")  # Unsupported
    # openssl = ("acecurity.crypto._openssl", "OpenSSL backend")  # Unsupported
    std_lib = ("acecurity.crypto._std_lib", "Standard library fallbacks")
...
class _BASIC_HASHER(metaclass=ABCMeta):
    ...
    @abstractmethod  # , text_ids: bool = True
    def verify(self, to_verify: bytes, original_hash: bytes, *args: _ty.Any, **kwargs: _ty.Any) -> bool:
        """
...
class _SPHINCS_KEYPAIRTYPE(
    ...
    def new(cls, mode: _SPHINCS_MODELITERAL) -> _te.Self:
        if not cls.__concrete__:
            raise NotSupportedError(f"{cls.cipher} not supported by this backend")
